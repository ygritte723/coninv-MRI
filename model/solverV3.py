...
class mae_trainer(nn.Module):
    ...
    def train_step(self, data, epoch):
        self.model.train()
        if self.s2s2:
            # unpack 10-tuple from forward_train
            (l1, l2,
             p1, p2,
             m1, m2,
             c_gl, c_gp,
            ...
            # Compute semantic consistency loss (sum of cosine similarity losses)
            semantic_sum = c_gl + c_gp + c_ll + c_lp
            # weighted total loss
            ...
            self.loss_dict = {
                'local_MSE':   l1.item(),
                'global_MSE':  l2.item(),
                'semantic_consistency': semantic_sum.item()            }
            ...
            local_loss, local_pred,  local_mask = \
            self.model.forward_train(
    ...
    def save_visualization(self, epoch):
        ...
        for k, v in self.visual_dict.items():
            if 'mask' in k:
                ...
                vis[k] = util.tensor2label(v[0, :, :], 2)  # No `D` index

            else:
                # vis[k] = util.tensor2im(v[0, :, :, :, slc_num])
                vis[k] = util.tensor2im(v[0, :, :])  # No `D` index
...
class mpl_trainer(nn.Module):
        ...
        for k, v in self.visual_dict.items():
            ...
            if 'masked_map' in k:
                ...
                vis[k] = util.tensor2label(
                    v.squeeze()[:].unsqueeze(0), self.cfg.train.cls_num)
            elif 'label' in k:
                vis[k] = util.tensor2label(
                    v.squeeze()[:].unsqueeze(0), self.cfg.train.cls_num)
                self.visual_dict[k] = v.squeeze().float()
                ...
                vis[k] = util.tensor2im(v[0,0, :, :])
                self.visual_dict[k] = v.squeeze().float()
            # print(f"Visualizing {k}: shape={v.shape}")  # Debug shapes
    ...
    def infer_single_scan(self, tmp_scans):
        ...
        pred = np.zeros((self.cfg.train.cls_num,) + tmp_scans.shape[:2])
        tmp_norm = np.zeros((self.cfg.train.cls_num,) + tmp_scans.shape[:2])

        scan_patches, _, tmp_idx = util.patch_slicer(tmp_scans, tmp_scans, self.cfg.data.patch_size,
                                                     (x - 16, y -
                                                      16), remove_bg=self.cfg.data.remove_bg, test=True, ori_path=None)
        # print("tmp_scans shape: ", tmp_scans.shape)   
        if len(tmp_scans.shape) == 2:  # If shape is (1, 256, 256)
            tmp_scans = np.expand_dims(tmp_scans, axis=-1)  # Add a channel dim (1, 256, 256, 1)

        # Ensure shape is (1, 256, 256, 2)
        if tmp_scans.shape[-1] == 1:  # If single-channel, duplicate to match expected shape
            tmp_scans = np.concatenate([tmp_scans, tmp_scans], axis=-1)  # Copy channel data
        ...
        for idx, patch in enumerate(scan_patches):
            ...
            location[:, patch_idx[0]:patch_idx[1], patch_idx[2]:patch_idx[3]] = 1

            sbj = tio.Subject(one_image=tio.ScalarImage(
                tensor=global_scan[:, bound[0]:bound[1], bound[2]:bound[3], :]),
                a_segmentation=tio.LabelMap(
                    tensor=location[:, bound[0]:bound[1], bound[2]:bound[3], :]))
            ...
            # print('shape of down_scan:', down_scan.shape)
            # print('shape of coordinates_A:', coordinates_A.shape)
            # print('shape of ipt:', ipt.shape)
            tmp_pred, _ = self.model(ipt, down_scan.cuda().reshape([1, 1, x, y]),
                                     coordinates_A)
        ...
        if pad_flag:
            ...
            assert pred_vol.shape == (
                x_ori_size, y_ori_size), 'pred_vol shape must be the same as the original scan shape'
        return pred_vol
