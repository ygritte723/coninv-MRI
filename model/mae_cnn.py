...
import numpy as np
from .lsc_loss import ConsistencyLoss
...
class MAE_CNN(nn.Module):
    ...
    def forward_train(self, local_patch, global_img=None, local_patch2=None, global_img2=None):
        # --- First View ---
        local_latent, local_mask = self.forward_encoder(local_patch, self.cfg.train.mask_ratio, self.cfg.train.local_mae_patch)
        local_pred = self.forward_local_decoder(local_latent)
        local_loss = self.recon_loss(local_patch, local_pred, local_mask)

        global_latent = global_mask = global_pred = global_loss = None
        if global_img is not None:
            global_latent, global_mask = self.forward_encoder(global_img, self.cfg.train.mask_ratio, self.cfg.train.global_mae_patch)
            global_pred = self.forward_local_decoder(global_latent)
            global_loss = self.recon_loss(global_img, global_pred, global_mask)

        # --- If no second view, return first view only ---
        ...
        # --- Second View ---
        local_latent2, local_mask2 = self.forward_encoder(local_patch2, self.cfg.train.mask_ratio, self.cfg.train.local_mae_patch)
        local_pred2 = self.forward_local_decoder(local_latent2)
        local_loss2 = self.recon_loss(local_patch2, local_pred2, local_mask2)

        global_latent2 = global_mask2 = global_pred2 = global_loss2 = None
        if global_img2 is not None:
            global_latent2, global_mask2 = self.forward_encoder(global_img2, self.cfg.train.mask_ratio, self.cfg.train.global_mae_patch)
            global_pred2 = self.forward_local_decoder(global_latent2)
            global_loss2 = self.recon_loss(global_img2, global_pred2, global_mask2)

        # --- Feature Pooling ---
        f_ll_1, f_ll_2 = self.avgpool(local_latent).flatten(1), self.avgpool(local_latent2).flatten(1)
        f_gl_1, f_gl_2 = self.avgpool(global_latent).flatten(1), self.avgpool(global_latent2).flatten(1)
        f_lp_1, f_lp_2 = local_pred.flatten(1), local_pred2.flatten(1)
        f_gp_1, f_gp_2 = global_pred.flatten(1), global_pred2.flatten(1)

        # --- Consistency Losses ---
        loss_fcb_ll = self.consist_loss(f_ll_1, f_ll_2)
        
        loss_c_gl = self.consist_loss(f_gl_1, f_gl_2)
        
        loss_fcb_lp = self.consist_loss(f_lp_1, f_lp_2)
